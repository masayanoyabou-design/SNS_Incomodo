...
│   ├── lib/                  アプリのコード（models/・services/・providers/・screens/・widgets/・theme/ に分離）
│   ├── test/                 業務ルールと画面の自動テスト（`flutter test` で実行）
│   ├── tool/rules_check.dart  セキュリティルールの検証（Firestoreエミュレータ必須）
...
- 開発環境の構築手順・ハマりどころは `MANUAL.md` の10〜23章を参照
